...
9. Of the person from the Cougars and the person with 11 goals,
...
data Assignment = MkAssignment { player :: Player, team :: Team, jersey :: Int, goal :: Int }
                    deriving (Eq)

instance Show Assignment where
  show MkAssignment {..} = "(" ++ show player ++ ", "
                            ++ show team ++ ", "
                            ++ show jersey ++ ", "
                            ++ show goal ++ ")"
...
answers = do
  solution@[douglas, mahoney, nates, pilgrim, quill]
    <- [zipWith4 MkAssignment players ts js gs
       ...
       , gs <- permutations goals]  -- has 1728000 permutations

  -- (1) Mahoney is either the person who wears number 26 or the player from the Icers
  Just icers <- [find (\p -> team p == Icers) solution]
  guard (jersey mahoney == 26 || jersey icers == 26)
  guard (team mahoney /= Icers)

  -- (2) Quill doesn't wear number 26
  guard (jersey quill /= 26)

  -- 3. Nates scored 2 goals more than Pilgrim.
  guard (goal nates - goal pilgrim == 2)

  -- 4. The player from the Red Boots scored 1 goal more
  -- than the person from the Warriors
  Just redboots <- [find (\p -> team p == RedBoots) solution]
  Just warriors <- [find (\p -> team p == Warriors) solution]
  guard (goal redboots - goal warriors == 1)

  -- 5. The player who wears number 25 plays for the Warriors.
  Just jersey25 <- [find (\p -> jersey p == 25) solution]
  guard (team jersey25 == Warriors)

  -- 6. The player who wears number 4
  -- scored 1 goal less than the player who wears number 26
  Just jersey4 <- [find (\p -> jersey p == 4) solution]
  Just jersey26 <- [find (\p -> jersey p == 26) solution]
  guard (goal jersey4 - goal jersey26 == (-1))

  -- 7. The person with 8 goals is from the Warriors
  -- Just goals8 <- [find (\p -> goal p == 8) solution]
  -- guard (team goals8 == Warriors)
  guard (goal warriors == 8)

  -- 8. Douglas scored 1 goal more than the player who wears number 25
  -- Just jersey25 <- [find (\p -> jersey p == 25) solution]
  guard (goal douglas - goal jersey25 == 1)
